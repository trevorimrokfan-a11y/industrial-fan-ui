...
const appImgs = [
  ...
  'https://images.unsplash.com/photo-1601004890684-d8cbf643f5f2?w=600&h=400&fit=crop&auto=format',
  'https://images.unsplash.com/photo-1543349689-9a4d426bee8e?w=600&h=400&fit=crop&auto=format',
]

const appIcons = [
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>,
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M1 6l11 6 11-6M1 12l11 6 11-6"/></svg>,
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>,
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83"/></svg>,
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>,
  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 17H3a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v5"/><circle cx="16.5" cy="18.5" r="2.5"/><circle cx="8.5" cy="18.5" r="2.5"/></svg>,
]
    ...
    <div className="pt-14">
      ...
      <section className="relative bg-[#0A0E1A] py-14 overflow-hidden border-b border-gray-800">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-xs text-gray-500 mb-3">{a.breadcrumb}</div>
          <h1 className="text-4xl md:text-5xl font-bold text-white">{a.pageTitle}</h1>
          <p className="text-gray-400 text-sm mt-3">{a.pageDesc}</p>
        </div>
      </section>
      ...
      <section className="py-14 bg-[#0A0E1A]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {a.items.map((app, i) => (
              <div key={i} className="group bg-[#111827] border border-gray-800 rounded-2xl overflow-hidden hover:border-blue-600/40 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="relative bg-gradient-to-r from-[#0F1A35] to-[#0A1428] border border-blue-900/40 rounded-2xl px-8 py-8 overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="relative z-10">
              <h2 className="text-xl md:text-2xl font-bold text-white">{a.ctaTitle}</h2>
              <p className="text-gray-400 text-sm mt-2">{a.ctaDesc}</p>
            </div>
            <Link to="/" className="relative z-10 btn-primary whitespace-nowrap">{a.contactTeam}</Link>
          </div>
        </div>
      </section>
    </div>
